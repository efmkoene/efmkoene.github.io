...
<a id="fn-1">[1]</a>: [↩](#fnref-1) In the literature, the factor $\mathbf{G}\mathbf{G}^{\mathsf{T}}/(N-1)$ is often used as the *unbiased* estimator of the covariance of $\mathbf{G}$, which tends to the identity matrix for large $N$ (often only when $N\gg n$). However, since $\mathbf{G}$ has zero mean (we know this *a priori*), no degree of freedom is used up to subtract a data-driven mean from our ensemble (you can see it -- at no point in our algorithm do we add or subtract anything from our ensemble), thus the correct factor is $1/N$; the literature is simply mistaken. Other literature might start off by drawing $N$ members first, subtracting their *mean* from each member to get to 'state vector deviations'; in such a case we should use the unbiased estimator of the covariance. Their derivation will then be identical to what we arrived at, save for the factor $(N-1)^{-1}$ rather than $N^{-1}. However, as we are not doing such a statistical trick, and merely approximate $\mathbf{P}_b$ in the Kalman filter, I don't see how a factor $(N-1)^{-1}$ should show its head. Of course, the practical difference is negligible for $N > 100$.