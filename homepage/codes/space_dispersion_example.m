...
short = 5;                 % [-]   Subsampling of dt (1/short * dt)
...
for j=2:M
    ...
    u2_l(xs) =u2_l(xs) + fs(j)*dt^2/dx*c*2; % Inject source
        
    % Update step
    uj_l(X) = r^2 * ( u2_l(X-1)-2*u2_l(X)+u2_l(X+1) ) + 2*u2_l(X) - u1_l(X);
    ...
    % Update step
    uj_s(X2) = r^2*short^2 * ( u2_s(X2-1)-2*u2_s(X2)+u2_s(X2+1) ) + 2*u2_s(X2) - u1_s(X2);
    ...
    subplot(3,1,2)
    plot(x2,uj_s),ylim([-1 1]),title(sprintf('Short dx=%0.2f m; time t=%0.2f s',dx/short,j*dt)),xlabel('offset [m]')
    hold on
