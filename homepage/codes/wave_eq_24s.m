...
f      = -ricker(20 , T-0.06            ) / 2; % Monopole source
g      =  ricker(20 , T-0.06+(dx/c-dt)/2) / 2; % Dipole source
...
for j=2:M
   ...
   % --------> Taylor series implementation as:
    ...
    % Plot
    %======================================================================
    plot(x,p),ylim([-1 1]),title(sprintf('Staggered grid, 2nd order time-4th space, time t=%0.2f',j*dt)),xlabel('offset [m]')
    hold on
